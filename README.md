# HDBENCH.NET Software

Windows / iOS / Android / Chrome 向けのソフトウェアを公開しています。
現行アプリは各配布先（GitHub / App Store / Google Play / Chrome ウェブストア）から、
旧作アプリは本リポジトリの **[Releases](https://github.com/HDBENCH/software/releases)** からダウンロードできます。

*English description follows the Japanese tables below.*

## 現行アプリ
| アプリ | 対応 | 概要 | 配布先 |
|--------|------|------|--------|
| **Motroxia** | iOS / Android | 車両のOBD2ポートに接続して各種情報をモニタリングする車載モニターアプリ | [App Store](https://apps.apple.com/app/motroxia/id6803079954) / [Google Play](https://play.google.com/store/apps/details?id=net.hdbench.motroxia) |
| **E-DnsProxyService** | Windows | 任意のDNSリクエストを許可／拒否できるDNSプロキシサービス | [GitHub](https://github.com/HDBENCH/E-DnsProxyService) |
| **ADSExplorer** | Windows | NTFSの代替データストリーム（ADS）をエクスプローラーで表示・削除できるシェル拡張 | [GitHub](https://github.com/HDBENCH/ADSExplorer) |
| **KTMouseGestures** | Chrome拡張 | Google Chrome用のシンプルなマウスジェスチャー拡張 | [Chromeウェブストア](https://chromewebstore.google.com/detail/kt-mouse-gestures/gfimipfmbiohbdjmbmmljponpjmhhokl) / [GitHub](https://github.com/HDBENCH/KTMouseGestures) |

## ベンチマーク
| アプリ | 種別 | 概要 |
|--------|------|------|
| **[HDBENCH](hdbench/)** | フリーソフト | PC性能の総合ベンチマーク（CPU/メモリ/ディスク/ビデオ） |
| **[FDBENCH](fdbench/)** | フリーソフト | ストレージ（HDD/USB/フラッシュ）性能ベンチマーク |
| **[DDBENCH](ddbench/)** | フリーソフト | DirectDrawの描画性能ベンチマーク |

## キャプチャ／録画
| アプリ | 種別 | 概要 |
|--------|------|------|
| **[DxRec2](dxrec2/)** | シェアウェア（販売終了） | DirectX/OpenGLアプリのFPS表示・動画録画 |
| **[DxRec](dxrec/)** | シェアウェア（販売終了） | FPS表示・ベンチマーク・録画（DxRec2の前世代） |

## ユーティリティ
| アプリ | 種別 | 概要 |
|--------|------|------|
| **[E-WordChanger 2000](ewordchanger/)** | シェアウェア（販売終了） | 英語版アプリのメニュー/ダイアログを日本語化 |
| **[LANDE-RD](landerd/)** | フリーソフト | 東芝製HDD＆DVDレコーダー用ネットダビング仮想HDD |

## その他
| アプリ | 種別 | 概要 |
|--------|------|------|
| **[E-WordChanger 2005](other/)** | シェアウェア（販売終了 / 開発停止） | E-WordChanger 2000 の後継（表示前変換に改良） |
| **[E-IPConfig](other/)** | フリーソフト | `ipconfig.exe` のGUI版（release/renew対応） |

---

# HDBENCH.NET Software (English)

Software for Windows / iOS / Android / Chrome.
Current apps are distributed through their own channels (GitHub / App Store / Google Play / Chrome Web Store);
classic apps can be downloaded from this repository's **[Releases](https://github.com/HDBENCH/software/releases)**.

## Current apps
| App | Platform | Description | Link |
|-----|----------|-------------|------|
| **Motroxia** | iOS / Android | A car monitor app that reads vehicle data through the OBD2 port | [App Store](https://apps.apple.com/app/motroxia/id6803079954) / [Google Play](https://play.google.com/store/apps/details?id=net.hdbench.motroxia) |
| **E-DnsProxyService** | Windows | A DNS proxy service that lets you allow or reject any DNS request | [GitHub](https://github.com/HDBENCH/E-DnsProxyService) |
| **ADSExplorer** | Windows | A shell extension that shows and deletes NTFS Alternate Data Streams (ADS) in Windows Explorer | [GitHub](https://github.com/HDBENCH/ADSExplorer) |
| **KTMouseGestures** | Chrome ext. | A simple mouse gesture extension for Google Chrome | [Chrome Web Store](https://chromewebstore.google.com/detail/kt-mouse-gestures/gfimipfmbiohbdjmbmmljponpjmhhokl) / [GitHub](https://github.com/HDBENCH/KTMouseGestures) |

## Benchmarks
| App | Type | Description |
|-----|------|-------------|
| **[HDBENCH](hdbench/)** | Freeware | Overall PC benchmark (CPU / memory / disk / video) |
| **[FDBENCH](fdbench/)** | Freeware | Storage (HDD / USB / flash) benchmark |
| **[DDBENCH](ddbench/)** | Freeware | DirectDraw rendering benchmark |

## Capture / Recording
| App | Type | Description |
|-----|------|-------------|
| **[DxRec2](dxrec2/)** | Shareware (discontinued) | FPS overlay & video recording for DirectX/OpenGL apps |
| **[DxRec](dxrec/)** | Shareware (discontinued) | FPS overlay, benchmark & recording (predecessor of DxRec2) |

## Utilities
| App | Type | Description |
|-----|------|-------------|
| **[E-WordChanger 2000](ewordchanger/)** | Shareware (discontinued) | Translates English app menus/dialogs into Japanese |
| **[LANDE-RD](landerd/)** | Freeware | Virtual HDD for Toshiba HDD&DVD recorder network dubbing |

## Others
| App | Type | Description |
|-----|------|-------------|
| **[E-WordChanger 2005](other/)** | Shareware (discontinued / dev stopped) | Successor to E-WordChanger 2000 (translates before display) |
| **[E-IPConfig](other/)** | Freeware | GUI version of `ipconfig.exe` (supports release/renew) |

---
本ソフトウェアは無保証です。自己責任でご利用ください。
This software is provided without warranty. Use at your own risk.
(C) 2001-2026 HDBENCH.NET
